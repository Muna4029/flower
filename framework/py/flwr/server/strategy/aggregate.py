...
def aggregate_inplace(results: list[tuple[ClientProxy, FitRes]]) -> NDArrays:
    ...
    def _try_inplace(
        x: NDArray, y: Union[NDArray, np.float64, float], np_binary_op: np.ufunc
    ) -> NDArray:
        y_array: np.ndarray[Any, Any] = np.asarray(y)
        return cast(
            NDArray,
            (
                np_binary_op(x, y_array, out=x)
...
def _compute_distances(weights: list[NDArrays]) -> NDArray:
    ...
    """
    flat_w = [cast(NDArray, np.ravel(np.concatenate(p, axis=None))) for p in weights]
